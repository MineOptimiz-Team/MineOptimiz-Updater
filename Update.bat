...
echo 正在删除旧的文件
set "MOD=.\mods"
powershell -Command "Get-ChildItem -Path '%MOD%' -Exclude 'exordium*.jar', 'enityculling*.jar', 'fancymenu*.jar', 'notenoughanimations*.jar', 'viafabricplus*.jar', 'voicechat*.jar' -Recurse | Remove-Item -Force -Recurse"
rd /s /q config
