...
if ($updatepack -eq "1") {
    Invoke-WebRequest -Uri "stabledownloadlink" -OutFile "./Update.zip" # 此处 stabledownloadlink 填写一个可下载稳定版整合包的直链
}
if ($updatepack -eq "2") {
    Invoke-WebRequest -Uri "devdownloadlink" -OutFile "./Update.zip" # 此处 devdownloadlink 填写一个可下载测试版整合包的直链
}
if ($? -eq $false) {
    echo "下载失败！"
    Pause
